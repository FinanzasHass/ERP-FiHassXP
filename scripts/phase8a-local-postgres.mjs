...
try{
 ...
 await mkdir('docs/fase-8a',{recursive:true});
 const migrationHashes={};for(const name of (await readdir('supabase/migrations')).filter(n=>/^2026091300(38|39|40|41|42|43)_/.test(n)))migrationHashes[name]=createHash('sha256').update(await readFile('supabase/migrations/'+name)).digest('hex');
 const resultPath='docs/fase-8a/resultado-postgres-local.json';
 await mkdir('docs/fase-8a/historico',{recursive:true});
 try{const previous=await readFile(resultPath,'utf8');await writeFile('docs/fase-8a/historico/postgres-'+Date.now()+'.json',previous);}catch(error){if(error.code!=='ENOENT')throw error;}
