...
const run=promisify(execFile),dir='docs/fase-8a';await mkdir(dir+'/capturas',{recursive:true});
...
try{
 ...
 const files=(await readdir('supabase/migrations')).filter(n=>/^2026091300(38|39|40|41|42|43)_/.test(n));
 if(files.length!==6)throw new Error('Unexpected migration set');
